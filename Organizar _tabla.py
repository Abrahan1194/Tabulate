...
for usuario in usuarios.values():
    ...
    direccion = f"{usuario['contacto']['direccion']['calle']}, {usuario['contacto']['direccion']['ciudad']}, {usuario['contacto']['direccion']['estado']}, {usuario['contacto']['direccion']['codigo_postal']}, {usuario['contacto']['direccion']['pais']}"
    fecha_nacimiento = usuario['perfil']['fecha_nacimiento']
    genero = usuario['perfil']['genero']
    ocupacion = usuario['perfil']['ocupacion']
    redes_sociales = f"Twitter: {usuario['redes_sociales']['twitter']}, LinkedIn: {usuario['redes_sociales']['linkedin']}, Instagram: {usuario['redes_sociales']['instagram']}"
    idioma = usuario['preferencias']['idioma']
    newsletter = "Sí" if usuario['preferencias']['newsletter'] else "No"#Si es True, asigna "Sí" a la variable newsletter, y si es False, asigna "No".
    temas_interes = ", ".join(usuario['preferencias']['temas_interes'])#La función .join() se utiliza para combinar todos los elementos de la lista en una cadena, separada por comas.

    # Agregar una fila para cada usuario
    tabla.append([nombre, apellido, correo, telefono, direccion, fecha_nacimiento, genero, ocupacion, redes_sociales, idioma, newsletter, temas_interes])

# Imprimir la tabla
#headers es la lista de los encabezados de las columnas.
headers = ["Nombre", "Apellido", "Correo", "Teléfono", "Dirección", "Fecha Nacimiento", "Género", "Ocupación", "Redes Sociales", "Idioma", "Newsletter", "Temas de Interés"]
print(tabulate(tabla, headers=headers, tablefmt="grid"))#tablefmt="grid" especifica que la tabla debe tener bordes de cuadrícula, haciendo que se vea más estructurada.
